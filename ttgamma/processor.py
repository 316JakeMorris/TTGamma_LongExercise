...
def selectMuons(events):
    ...
    """
    muonSelectTight = (
        (events.Muon.pt > 30) * (abs(events.Muon.eta) < 2.4) * (events.Muon.tightId) * (events.Muon.pfRelIso04_all < 0.15)
    )
...
def selectPhotons(photons):
    ...
    tightPhotons = photons[photonSelect * (photons.cutBased >= 4) ]
    ...
    loosePhotons = photons[photonSelect * photonID_NoChIso * (photons.cutBased >= 1)]
