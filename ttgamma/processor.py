...
def selectMuons(events):
    ...
    """
    muonSelectTight = (
        (events.Muon.pt > 30) & (abs(events.Muon.eta) < 2.4) & (events.Muon.tightId) & (events.Muon.pfRelIso04_all < .15)
    )  # FIXME 1a FIXED? May have to add tightId == True
    ...
    def process_shift(self, events, shift_syst=None):
        ...
        if self.isMC:
            ...
            if shift_syst == "JERUp":
                ...
                jets = corrected_jets.JES_jes.up  # FIXME 4 FIXED
            elif shift_syst == "JESDown":
                jets = corrected_jets.JES_jes.down  # FIXME 4 FIXED
            else:
        ...
        tightJet["btagged"] = tightJet.btagDeepB > bTagWP  # FIXME 1a FIXED
        ...
        selection.add("eleTrigger", events.HLT.Ele27_WPTight_Gsf)  # FIXME 1b FIXED
        ...
        selection.add("zeroMuon", ak.num(tightMuons) == 0) # FIXME 1b FIXED
        # we also need to know if there are any loose muons in each event
        selection.add("zeroLooseMuon", ak.num(looseMuons) == 0) # FIXME 1b FIXED

        # similar selections will be needed for electrons
        selection.add("oneEle", ak.num(tightElectrons) == 1)  # FIXME 1b FIXED
        selection.add("zeroEle", ak.num(tightElectrons) == 0)  # FIXME 1b FIXED
        selection.add("zeroLooseEle", ak.num(looseElectrons) == 0)  # FIXME 1b FIXED
        ...
        selection.add("zeroPho", ak.num(tightPhotons) == 0)  # FIXME 1b FIXED

        # add selection for events with exactly 1 tight photon
        selection.add("onePho", ak.num(tightPhotons) == 1)  # FIXME 1b FIXED

        # add selection for events with exactly 1 loose photon
        selection.add("loosePho", ak.num(loosePhotons) == 1)  # FIXME 1b FIXED
        ...
        if self.isMC:
            ...
            puWeight_Up = puLookup_Up[datasetFull](events.Pileup.nTrueInt)  # FIXME 4 FIXED
            puWeight_Down = puLookup_Down[datasetFull](events.Pileup.nTrueInt)  # FIXME 4 FIXED
            ...
            muSF_down = ak.prod((muID - muIDerr) * (muIso - muIsoerr) * (muTrig - muTrigerr), axis=-1)  # FIXME 4 FIXED
            weights.add("muEffWeight", weight=muSF, weightUp=muSF_up, weightDown=muSF_down)  # FIXME 4 FIXED
